    ...
    <div className="w-full h-[350px] bg-white rounded-xl shadow-sm p-4 border border-slate-100">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          />
          <ReferenceLine y={average} label="Avg" stroke="#0d9488" strokeDasharray="3 3" />
          <Area type="monotone" dataKey="radiation" name="Solar Radiation" stroke="#f59e0b" fillOpacity={1} fill="url(#colorRad)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export const WeatherChart: React.FC<{ data: SolarDataPoint[] }> = ({ data }) => {
  return (
    <div className="w-full h-[350px] bg-white rounded-xl shadow-sm p-4 border border-slate-100 mt-6">
      <h3 className="text-lg font-semibold text-slate-700 mb-4">Weather Conditions (Temp & Clouds)</h3>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          />
          <YAxis yAxisId="left" tick={{ fontSize: 12, fill: '#64748b' }} label={{ value: 'Temp (°C)', angle: -90, position: 'insideLeft' }} />
          <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: '#64748b' }} label={{ value: 'Cloud Cover (%)', angle: 90, position: 'insideRight' }} />
          <Tooltip 
            contentStyle={{ backgroundColor: '#fff', borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
          />
          <Legend />
          <Area yAxisId="right" type="monotone" dataKey="cloudCover" name="Cloud Cover %" fill="#e2e8f0" stroke="#94a3b8" fillOpacity={0.4} />
          <Line yAxisId="left" type="monotone" dataKey="temperature" name="Temp (°C)" stroke="#ef4444" strokeWidth={2} dot={false} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
