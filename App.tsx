...
import { RadiationChart, RotationChart, WeatherChart } from './components/SolarCharts';
...
const App: React.FC = () => {
  ...
  // Map Default Center (Rajkot)
  const [mapCenter, setMapCenter] = useState({ lat: 22.3039, lon: 70.8022 });
  ...
  useEffect(() => {
    ...
    // Set initial coordinates for Rajkot so user can proceed immediately if they want
    setInputs(prev => ({ ...prev, coordinates: { lat: 22.3039, lon: 70.8022 } }));
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      // 1. Fetch Historical Solar Data (NASA)
      ...
      if (solarData.length === 0) {
        throw new Error("Failed to fetch solar data for this date range.");
      }

      // 2. Fetch Current Weather (Open-Meteo)
      ...
      // Calculations
      const radValues = solarData.map(d => d.radiation);
      const tempValues = solarData.map(d => d.temperature);
      const cloudValues = solarData.map(d => d.cloudCover);

      const avgRad = radValues.reduce((a, b) => a + b, 0) / radValues.length;
      const avgTemp = tempValues.reduce((a, b) => a + b, 0) / tempValues.length;
      const avgCloud = cloudValues.reduce((a, b) => a + b, 0) / cloudValues.length;
      ...
      // Efficiency Drop due to temp (simplified: -0.4% per degree over 25C)
      const tempLossFactor = avgTemp > 25 ? 1 - ((avgTemp - 25) * 0.004) : 1;
      
      const energyPerPanel = avgRad * panelEfficiency * panelArea * rotationFactor * tempLossFactor;
        ...
        {step === AppStep.RESULTS && analysis && (
          <div className="animate-fadeIn">
            ...
            {analysis.currentWeather && (
               <CurrentWeather data={analysis.currentWeather} locationName={analysis.location.name} />
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-orange-400">
                <p className="text-xs font-bold text-slate-400 uppercase">Avg Temp</p>
                <h3 className="text-2xl font-bold text-slate-800 mt-1">{analysis.averageTemperature.toFixed(1)} <span className="text-sm font-normal text-slate-500">°C</span></h3>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="lg:col-span-2 space-y-6">
                ...
                <WeatherChart data={analysis.dailyData} />
              </div>
              <div className="lg:col-span-1">
                {inputs.useRotation ? (
                   <RotationChart lat={analysis.location.lat} />
                ) : (
                  <div className="h-[350px] bg-white rounded-xl shadow-sm p-6 border border-slate-100 flex flex-col items-center justify-center text-center">
